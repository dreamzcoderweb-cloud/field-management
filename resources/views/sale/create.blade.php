...
@section('main-content')
    ...
    <form action="{{ route('sale.store') }}" method="post" enctype="multipart/form-data" id="formValidation">
        @csrf
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            <div class="form-group col-md-4 mb-3">
                                ...
                                <div class="row qty" style="display: none">
                                    <div class="col-md-12 d-flex">
                                        <span class="text-dark me-2">Quantity:</span>
                                        <p id="quantity" class="mb-0"></p>
                                    </div>
                                </div>
    ...
    <label class="control-label mb-2">Payment Method</label>
    ...
    <div class="d-flex gap-4">
        <div class="form-check">
            <input class="form-check-input payment_method" type="radio"
                ...
                value="1"
                {{ (string) old('payment_method', '1') === '1' ? 'checked' : '' }}>
        ...
        <div class="form-check">
            <input class="form-check-input payment_method" type="radio"
                ...
                value="0"
                {{ in_array((string) old('payment_method'), ['0', 'bank'], true) ? 'checked' : '' }}>
    ...
    <span class="text-danger">{{ $errors->first('payment_method') }}</span>
                            ...
                            <div class="form-group col-md-4 mb-3 payment_bank" hidden>
...
<div class="form-group col-md-6 mb-3 payment_bank" hidden>

                                <label for="interest" class="control-label">Interest <small class="text-danger">(in
                                        %)</small></label>
                                <input id="interest" name="interest" class="form-control" value="{{ old('interest') }}"
                            ...
                            <div class="form-group col-md-2 mb-3">
                                ...
                                <input id="product_amount" name="product_amount" class="form-control" value="{{ old('product_amount') }}"
                                ...
                                <div class="col-md-6">
                                    <div class="row">
                                        <div class="form-group col-md-3 mb-3 exchangable_item" hidden>
                                            ...
                                            <span
                                                class="text-danger">{{ $errors->first('exchangable_item', ':message') }}</span>
                                        </div>
                                        ...
                                        <div class="form-group col-md-3 mb-3 exchangable_item" hidden>
                                            <label for="vehicle" class="control-label">Vehcile</label>
                                            <input id="vehicle" name="vehicle" class="form-control"
                                                value="{{ old('vehicle') }}" />
                                            <span
                                                class="text-danger">{{ $errors->first('vehicle', ':message') }}</span>
                                        </div>

                                        <div class="form-group col-md-3 mb-3 exchangable_item" hidden>
                                            <label for="vehicle_number" class="control-label">Vehcile Number</label>
                                            <input id="vehicle_number" name="vehicle_number" class="form-control"
                                                value="{{ old('vehicle_number') }}" />
                                            <span
                                                class="text-danger">{{ $errors->first('vehicle_number', ':message') }}</span>
                                        </div>

                                        <div class="form-group col-md-3 mb-3 exchangable_item" hidden>
                                            <label for="vehicle_year" class="control-label">Vehcile Year</label>
                                            <input id="vehicle_year" name="vehicle_year" class="form-control"
                                                value="{{ old('vehicle_year') }}"
                                                oninput="this.value = this.value.replace(/[^0-9]/g, ''); if (this.value.length > 4) this.value = this.value.slice(0, 4);" />
                                            <span
                                                class="text-danger">{{ $errors->first('vehicle_year', ':message') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="form-group d-flex justify-content-end">
                ...
                <a href="{{ route('sale.index') }}" class="btn btn-danger" style="margin-left: 4px;">Back</a>
            </div>
    ...
    $(document).ready(function() {
        $('#formValidation').validate({
            rules: {
                ...
                bank_id: {
                    required: function() {
                        return $('input[name="payment_method"]:checked').val() === '0';
                    }
                },
                interest: {
                    required: function() {
                ...
                advance_amount: {
                    required: function(element) {
                        return $('input[name="paid_advance"]:checked').val() === '1';
                    }
                },
                is_exchangable: "required",
                exchangable_item: {
                    required: function(element) {
                        return $('input[name="is_exchangable"]:checked').val() === '1';
                    }
                },
                exchangable_amount: {
                    required: function(element) {
                ...
                emi_month: {
                    required: function(element) {
                        return $('input[name="emi_applicable"]:checked').val() === '1';
                    }
                },
                emi_date: {
                    required: function(element) {
            ...
            messages: {
                ...
                vehicle_number: "The Vehicle Number is required",
                vehicle_year: "The Year is required",
        ...
        function togglePaymentFields() {
            ...
            const isBank = method === '0';

            if (isBank) {
                $('.payment_bank').attr('hidden', false);
                $('#interest_amount').closest('.form-group').attr('hidden', false);
            } else {
                $('.payment_bank').attr('hidden', true);
                $('#interest').val('');
                $('#interest_amount').val('0.00');
                $('#interest_amount').closest('.form-group').attr('hidden', true);
            }
            calculateInterest();
            calculateAmounts();
        }
        ...
        function calculateInterest() {
            ...
            const isBank = $('input[name="payment_method"]:checked').val() === '0';
            if (!isBank) {
                interest = 0;
            }
        ...
        function calculateAmounts() {
            let total_amount = parseFloat($('#amount').val()) || 0;
            let hasAdvance = $("input[name='paid_advance']:checked").val() === '1';
            let advance_amount = parseFloat($("#advance_amount").val()) || 0;
            let paid_amount = hasAdvance ? advance_amount : 0;
            let balance = Math.max(total_amount - paid_amount, 0);

            $("#paid_amount").val(paid_amount);
            $("#balance").val(balance);
        }

        $("input[name='payment_method']").on('change', calculateAmounts);
        $("#advance_amount").on('input', calculateAmounts);
        ...
        $('#emi_month').on('input', function() {
            ...
            let emi_amount = emi_month > 0 ? Math.round(balance / emi_month) : 0;
