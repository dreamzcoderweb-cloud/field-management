...
@section('main-content')
    ...
    <form action="{{ route('admin.vehiclemanagement.update', $vehiclemanagement->id) }}" method="post" id="formValidation">
        @csrf
        @method('PUT')
                        ...
                        <div class="form-group row">
                                ...
                                <select name="vehicle_id" class="form-control">
                                    ...
                                    @foreach ($vehicles as $vehicle)
                                        <option value="{{ $vehicle->id }}"
                                            {{ old('vehicle_id', $vehiclemanagement) == $vehicle->id ? 'selected' : '' }}>
                                            {{ $vehicle->name }} -
                            ...
                            <div class="form-group col-md-6 mb-3">
                                ...
                                <input id="driver_name" name="driver_name" class="form-control"
                                    value="{{ old('driver_name', $vehiclemanagement) }}" />
                            ...
                            <div class="form-group col-md-4 mb-3">
                                ...
                                <input id="location" name="location" class="form-control"
                                    value="{{ old('location', $vehiclemanagement) }}" />
                            ...
                            <div class="form-group col-md-4 mb-3">
                                ...
                                <input id="distance" name="distance" class="form-control"
                                    value="{{ old('distance', $vehiclemanagement) }}"
                            ...
                            <div class="form-group col-md-4 mb-3">
                                <label for="fuel" class="control-label">Fuel <small class="text-danger">in
                                        Liter</small></label>
                                <input id="fuel" name="fuel" class="form-control"
                                    value="{{ old('fuel', $vehiclemanagement) }}"
                                    oninput="this.value = this.value.replace(/[^0-9.]/g, ''); if (this.value.length > 10) this.value = this.value.slice(0, 10);" />
                                <span class="text-danger">{{ $errors->first('fuel', ':message') }}</span>
                            </div>
                            <div class="form-group col-md-12 mb-3">
                                <label for="notes" class="control-label">Notes</label>
                                <textarea id="notes" name="notes" rows="3" class="form-control">{{ old('notes', $vehiclemanagement) }}</textarea>
