...
class ExchangeSalesReport extends Controller
{
    /**
    ...
    {
        $sales = $this->salesQuery(request()->all(), 1)
            ->latest()
            ->paginate(10)
            ->withQueryString();
        ...
        $clients = Client::get();
        $teams = Team::get();
        return view('exchange_report.sale', ['sales' => $sales, 'products' => $products, 'clients' => $clients, 'teams' => $teams]);
    }
    ...
    {
        return view('exchange_report.saledetail', ['sale' => $sale->load(['product', 'client', 'bank', 'due', 'duty'])]);
    }

    public function export(Request $request)
    {
        return Excel::download(
            new SalesReportExport($request->all(), 1),
            now()->format('Ymd_His') . '_exchange_sale_report.xlsx'
        );
    }
